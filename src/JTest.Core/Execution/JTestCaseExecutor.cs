...
public sealed class JTestCaseExecutor(IStepProcessor stepProcessor) : IJTestCaseExecutor
{
    /// <summary>
    ...
    /// </summary>
    private async Task<List<JTestCaseResult>> RunTestWithDatasetsAsync(JTestCase testCase, TestExecutionContext baseContext, int testNumber, CancellationToken cancellationToken)
    {
        var results = new List<JTestCaseResult>();
        ...
        foreach (var dataset in testCase.Datasets!)
        {
            ...
            var result = await ExecuteTestCaseAsync(testCase, iterationContext, dataset, cancellationToken);
            results.Add(result);
    ...
    /// </summary>
    private async Task<JTestCaseResult> ExecuteTestCaseAsync(JTestCase testCase, TestExecutionContext executionContext, JTestDataset? dataset, CancellationToken cancellationToken)
    {
    ...
    /// </summary>
    private static Dictionary<string, object?> CaptureOriginalVariables(TestExecutionContext baseContext)
    {
    ...
    /// </summary>
    private static TestExecutionContext PrepareIterationContext(
        ...
        Dictionary<string, object?>? sharedGlobals)
    {
        ...
        {
            iterationContext.Variables[kvp.Key] = kvp.Key.ToLowerInvariant() switch
            {
                "env" => kvp.Value,// env is immutable - use original reference (no cloning needed)
                "globals" => sharedGlobals ?? kvp.Value,// globals are shared across iterations - use updated value if available
                _ => DeepCloneVariable(kvp.Value),// all other variables (ctx, this, named variables) - reset to original
            };
        }
        ...
        {
            iterationContext.Variables["globals"] = sharedGlobals ?? [];
        }
