...
namespace JTest.Core.Output.Markdown
{
    public sealed class MarkdownTestCaseResultWriter : ITestCaseResultWriter
    {
        ...
        void WriteStepResult(TextWriter writer, StepResult stepResult, bool isDebug, string? templateName)
        {
            ...
            var useStep = stepResult.Step as UseStep;
            if (useStep is not null)
            {
                writer.WriteLine($"**Template**: {useStep.Template}  <br/>");
            }
            ...
            if (stepResult.InnerResults.Count > 0)
            {
                writer.WriteLine();
                writer.WriteLine();
                writer.WriteLine($"**Inner step results for template**: '{useStep?.Template}' <br/>");
                writer.WriteLine();

                foreach (var innerStepResult in stepResult.InnerResults)
                {
                    WriteStepResult(writer, innerStepResult, isDebug, useStep?.Template);
                }
            }
        }

        void WriteAssertionsTable(TextWriter writer, IEnumerable<AssertionResult> assertions)
        {
        ...
        static void WriteAssertionColumn(TextWriter writer, string value)
        {
        ...
        static void WriteAsserionTableHeader(TextWriter writer, string header)
        {
            writer.WriteLine($"<th style=\"border: 1px solid #333; padding: 8px; width: 150px;\">{header}</th>");

        }

        static string ParseObjectValue(object? value, bool maskValue)
        {
            if (value is null)
            {
                return string.Empty;
