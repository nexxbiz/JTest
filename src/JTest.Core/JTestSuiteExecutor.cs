...
{
    public sealed class JTestSuiteExecutor(JTestCaseExecutor testCaseExecutor, IVariablesContext variablesContext, ITemplateContext templateContext)
    {
        public async Task<IEnumerable<TestFileExecutionResult>> Execute(IEnumerable<JTestSuite> testFiles)
        {
        ...
        public IEnumerable<TestFileExecutionResult> ExecuteParallel(IEnumerable<JTestSuite> testFiles, int parallelCount)
        {
            // Thread-safe collections for parallel execution            
            var allResults = new ConcurrentBag<TestFileExecutionResult>();
