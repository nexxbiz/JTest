...
default. A formal, versioned JSON Schema for the JTest language replaces the shallow structural
...
| IV | Secure & redacted by default | One `ReportValuePipeline` performs contextual encoding + value-based redaction for every projected value; env/global dumps opt-in and masked. Escaping/redaction tests required. | PASS |
...
| VI | Pipeline-first & deterministic | Self-contained HTML; deterministic node ordering; cancellation/timeout are distinct outcomes; sequential/parallel equivalence test. | PASS |
...
├── contracts/           # Phase 1 output — CLI, trace schema, language schema contracts
│   ├── cli-contract.md
...
│   ├── Execution/                   # REWORK — suite/case/step executors capture all outcomes
│   ├── Steps/                       # REWORK — ForLoopStep/WhileStep iteration retention, step ids
│   ├── Language/
...
3. **Execution correctness (US1, US4 — P1/P2)** — rework executors/steps to emit the trace,
   ...
   ids/ordinals; `ExitCodeService` with class-specific codes; parallel==sequential. Tests for
   false-green, loops, nesting, cancellation, timeout, exit codes. (FR-001–FR-008, FR-013–FR-014)
4. **Report value pipeline & security (US3 — P2)** — `ReportValuePipeline` (encode + redact by
   value & key, bodies included); fix `SecurityMasker`; opt-in masked env/global dumps. Tests for
   XSS corpus and secret corpus. (FR-024–FR-028)
5. **HTML report projector (US2 — P1)** — self-contained single-file HTML from the trace:
   ...
   (FR-016–FR-023)
6. **CLI surface** — `run` emits HTML and (on request) canonical trace JSON; `report` command to
   render a report from a saved trace; wire exit codes; keep `debug` as an opt-in verbose view.
7. **Formal language schema & honest validation (US5 — P3)** — author versioned JSON Schema;
...
8. **Release integrity (US6 — P3)** — reconcile version across source/package/tag; README license
