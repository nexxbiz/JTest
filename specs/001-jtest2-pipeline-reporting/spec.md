...
- **FR-040**: HTTP response data MUST expose headers as a keyed map addressable by name in **lower case** (e.g. `headers['content-type']`), regardless of the casing the server sent; multi-valued headers (e.g. `set-cookie`) MUST expose all values. Request data MUST expose headers the same way. Because the execution context is serialized to JSON before JSONPath evaluation and RFC 9535 name selectors are case-sensitive (FR-048), a case-insensitive dictionary comparer does NOT satisfy this requirement at the layer authors use: keys MUST be normalized at capture. Verification MUST exercise the JSONPath path, not a direct dictionary lookup.
...
- **FR-049**: A JSONPath that matches nothing MUST be recorded as a distinct, visible diagnostic in the trace and report at its point of use (a `save` source or an assertion `actual`/`expected`), distinguishable from a path that matches an actual `null`. An unresolved path MUST NOT be silently coerced to `null` in a way that can mask a failure or produce a misleading pass. An assertion whose actual/expected contains an unresolved path MUST fail naming that path rather than comparing a coerced blank value; `exists`/`notexists` are exempt, since for them "matched nothing" is the answer under test. Verification MUST exercise the assertion and `save` paths, not only the resolver helper.
- **FR-049a**: The unresolved-path diagnostic SHOULD name the likely cause where it is knowable — in particular that RFC 9535 JSONPath has no `.length`/`.count`/`.size` property (a JavaScript-ism that resolves to nothing), and otherwise that name selectors are case-sensitive.

### Functional Requirements — Assertion operator contract

- **FR-054**: Operator operand validation MUST check the operand each operator actually consumes. `in` asks whether a **scalar** actual value is a member of a **collection** expected value ("one of these"), so it MUST accept a scalar (including numeric) actual and MUST validate that its `expectedValue` is a collection. Only `length`, `empty`, and `notempty` inspect the `actualValue` as a collection.
- **FR-055**: The documentation MUST carry a complete reference of the supported assertion operators and the operand each one expects, so an operator's semantics are discoverable without reading the source or the schema. `jtest validate` MUST reject an unrecognized operator with its location and the supported set. Because operators are matched case-insensitively at run time, this check MUST resolve against the runtime's operator registry rather than a fixed schema enum, so validation can never reject a spelling that would execute.
- **FR-064**: `jtest validate` MUST NOT report a definition as valid that `jtest run` cannot load. Schema conformance describes shape only, while the runner additionally binds the document to its configuration types; validation MUST therefore also perform that binding and report any failure as a located diagnostic. A validator that green-lights input the runner rejects moves a diagnosable authoring problem to run time and surfaces it as a crash rather than a diagnostic.
- **FR-065**: Header and query values carry text. A scalar (string, number, boolean, null) MUST be accepted for a `query` value and a request `headers` value and used as its textual form — `{ "take": 1 }` means `take=1`, which is what the author means. A value with no unambiguous textual form (object or array) MUST be rejected by validation, naming the key. Request `headers` MUST accept both the object-map shape (`{ "X-Tenant": "acme" }`) and the array shape (`[{ "name": ..., "value": ... }]`); the object map is the documented default and mirrors how the response contract exposes headers (FR-040).
- **FR-061**: An unresolvable `{{...}}` token in a **step field** (URL, method, header, query value, body value, file path, or template argument) MUST fail the step, naming the token. It MUST NOT be substituted with an empty string. Silent substitution is a correctness defect, not a cosmetic one: a suite that mints a per-run unique resource name instead sends a constant, so every run targets the same server-side resource and the suite can report success while being served by a previous run's artifact — a false positive that ordinary assertions cannot detect. This extends FR-049 from assertion/`save` operands to every value a step acts on.
- **FR-062**: JTest MUST provide the dynamic values `$.now` (`iso`, `date`, `time`, `epoch`, `epochMs`) and `$.random` (`uuid`, `id`), evaluated at each reference. These were documented as built-ins before they existed and expanded to an empty string, which is what produced the FR-061 failure mode in practice. Values defined by the suite itself under those roots MUST take precedence, so introducing the built-ins cannot change the meaning of an existing suite. A reference to a non-existent field under a built-in root MUST be an error naming the available fields, not an unresolved path.
- **FR-063**: Documentation that lives outside `docs/` but is read as repository guidance (e.g. files under `.github/`) is subject to FR-044: it MUST NOT describe behavior the shipped system does not have. A stale guide in an agent-visible location is read with the same authority as the real documentation and MUST be corrected or replaced with a pointer to `docs/`.

- **FR-060**: A suite that cannot be loaded (malformed JSON, unknown step type, unknown assertion operator) MUST be captured as an errored suite, exactly as a suite that crashes during execution (FR-002). Loading MUST NOT happen eagerly for all discovered files outside the per-suite boundary: one unloadable file MUST NOT prevent the other suites from executing, MUST NOT suppress the trace and report, and MUST NOT produce an exit code outside the documented contract (FR-001).

### Functional Requirements — Loop semantics

- **FR-056**: A `for` step over an empty item list MUST run zero iterations and succeed; an empty list is a normal state (e.g. "clean up whatever is left over" with nothing left), not a validation error. A `for` step MUST still require at least one inner step.

### Functional Requirements — Per-run generated values

- **FR-057**: JTest MUST expose generated values that are unique to the run under `$.run` — at minimum a short identifier safe for use in URLs and identifiers, a UUID, and the run start as an ISO-8601 timestamp and Unix epoch. This exists so a suite that creates a server-side resource with globally-unique identity (e.g. an HTTP route that the server treats as exclusive) can generate a fresh value per run and remain re-runnable, instead of passing once and conflicting on every later run.
- **FR-058**: `$.run` values MUST be stable for the whole run — every step, case, and suite in one invocation observes the same values — so a create step and a later fetch step agree without an intervening `save`.
- **FR-059**: The `$.run` values used by a run MUST be recorded in the canonical trace unconditionally (independent of the opt-in variable dump of FR-027/FR-028), so a run that derives identifiers from them remains reproducible from its evidence.
...
- **FR-053**: Environment variables given on the command line MUST take effect. Each `-e/--env key=value` (repeatable) MUST define `$.env.key` for the run and MUST override a suite-level `env` entry of the same name, exactly as `--env-file` does — a pipeline MUST be able to retarget a suite (e.g. a dynamically allocated `baseUrl`) without rewriting it. Only the **first** `=` separates key from value, so values may contain `=`; an entry with no key MUST be rejected with a usage error rather than silently dropped. When a key is supplied by both `--env-file` and `-e/--env`, the command line wins and the run MUST NOT abort on the duplicate.
...
- **Target runtime** remains .NET (net10.0) and the `jtest` dotnet global tool distribution; no change of platform or distribution channel is in scope.
